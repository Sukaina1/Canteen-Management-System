...
using namespace std;
//This functions sets cursor position to a praticular spot on the screen.
COORD coord = { 0,0 };
void gotoxy(int x, int y)
{
    coord.X = x;
    coord.Y = y;
    SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord); //SetConsoleCursorPosition uses that handle and the coordinates specified to place the blinking cursor in the command prompt.
}
//Global Variables.
...
class order;
class billing;
//Store Class manages the price and quantity of the food items.
class store
{
public:
    string item;
    ...
    int qty = 0;
    void mnginvt();
    void storepageswitch(); // Shows Menu of the  store.
    void price(); //Allows to set the price for the food items. 
    void viewinvt(); // Allows us to view the Food items along with their prices.
    void quantity(); //Allows to set the quantity of food items in our canteen.
    void storepage()
    {
        storepageswitch();
    }
...
// This class handles the purchase and billing of the customer. 
class billing
{
public:
    string name;
    ...
    int amt;
    char dec;
    void viewstat(); //It views the customer name along with the total amount paid.
    void clearstat();//It clears the customer details along with the billing status.
    billing()
    {
        cost1 = 0;
        cost2 = 0;
    ...
    void bill() //It shows a menu to the customer.
    {
        system("CLS");
        gotoxy(40, 1);
        cout << "THE PINNACLE";
...
//This class allows the customer to order the food items.
class order
{
public:
    int tqty;
    ...
    int ch;
    int qty;
    void orderitem1(); //Function to order the first item that is some sort of snack.
    void orderitem2(); //Function to order the second item that is maggi.
    void orderitem3(); //Function to order the third item that is soft drinks.
    void orderpageswitch(); //Function that gives options to the customer.
    void orderpage() //Menu page for the food items to be ordered.
    {
        system("CLS");
        gotoxy(40, 1);
        cout << "THE PINNACLE";
...
//Allows the owner to add,remove and display employee details along with view and clear sales record.
class employee
{
public:
    int ch;
    ...
    char name[50];
    long int sal;
    void addemp();// A function to add an employee to the canteen.
    void displayemp();// A function to display the information about the employee.
    void removeemp();//A function to remove the employee information.
    void emppageswitch();//A function that provides a options to choose from the menu(empage)
    void emppage()// A function that provides the menu to the owner.
    {
        system("CLS");
        gotoxy(40, 1);
        cout << "THE PINNACLE";
...
void employee::removeemp()
{
    ...
    cout << "     ENTER THE NAME OF EMPLOYEE YOU WISH TO REMOVE" << endl;
    cout << "     ";
    cin >> tname;
...
//Allows the user to login either as an owner or an employee.
class login
{
public:
    string pass = "";
    int ch;
    char c;
    void loginpageswitch(); //A function that provides a choice whether the user is an owner or an employee.
    void homepageswitch(); // A function that provides a choice whether the user wants to login or exit at the beginning.
    void employeelogin(); //A function that asks for employee login.
    void ownerlogin(); //A function that asks for owner login.
    void emp(); //A function that calls the store page function.
    void own(); //A function that calls the employee page function.
    void homepage()
    {
        system("CLS");
        gotoxy(40, 1);
        cout << "WELCOME TO THE PINNACLE";
        gotoxy(40, 2);
        cout << "----------------------------" << endl << endl;
        cout << "     1. LOGIN" << endl;
...
//A function that allows the customer to place a new order.
void billpage()
{
    int ch;
    system("CLS");
